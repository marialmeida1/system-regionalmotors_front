<template>
    <main class="content">
            ...
            <div class="pt-2 ps-2 pb-3">
                <!-- Links -->
                <div class="py-2 pt-3 px-2" style="font-size: 13px;">
                    <span class="me-2 p-1 px-2 rounded-3 me-2" style="background-color: rgba(82, 82, 82, 0.2); color: #000; font-weight: 600;" v-for="(filtro, index) in array_filtros" :key="index"> 

                        <span v-if="filtro.chave == 'situacao_veiculo'">
                            <span v-if="filtro.valor == '1'">
                                Novo
                            </span>
                            <span v-if="filtro.valor == '2'">
                                Usado
                            </span>
                        </span>
            ...
            <div class="pt-5 px-3" v-show="notFound">
                    <div class="col-lg-6 mx-auto rounded-3 text-center py-4" style="background-color: rgba(82, 82, 82, 0.2).2);">
                        <h4>Nenhum resultado!</h4>
                        <p class="m-0">Click para 
...
<div class="col-lg-8 col-md-8 p-0 m-0">
    <div>
                <div class="row p-0 m-0">
                <div class="col-lg-4 col-md-6 p-0 m-0 mb-3" v-for="(item, index) in $store.state.resultado" :key="index">
...
export default {
  ...
  methods: {
    ...
    async applyFiltro(chaveID, valorID) {
           ...
           // Use qs.stringify para converter os parâmetros em uma string de consulta
            ...
            const url = `api/anuncios/listar_anuncios?${queryString}&destaque_busca=1&status_publicacao=2`;

            // Chamada à API para filtrar anúncios com a URL construída
            this.$store.state.resultado = await api.filtrarAnuncio(url);
  ...
  async mounted() {
    ...
    this.$store.state.resultado = await api.filtrarAnuncio(`api/anuncios/listar_anuncios?${keys[0]}=${valor}&destaque_busca=1&status_publicacao=2`);
...
<style>
    ...
    .imagem-container2, .glide__slides {
        /*width: 110px;*/
        height: 150px;
        overflow: hidden;
        background: #fff;
...
@media (max-width: 568px) {
    .imagem-container2, .glide__slides, .imagem-container2 img {
        /*width: 110px;*/
        height: 130px;
    }
}
